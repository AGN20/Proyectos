...
class LigaPartido(models.Model):
            ...
            for recordPartido in self.env['liga.partido'].search([]):  
                ...
                if recordPartido.equipo_casa.nombre==recordEquipo.nombre:
                    ...
                    if recordPartido.goles_casa>recordPartido.goles_fuera:
                        # --ACTIVIDAD 01--
                        #Si los goles son 4 o más sumaran 4 Puntos (En caso de que gane el partido)
                        ...
                        # --ACTIVIDAD 01--
                        #Si los goles son 4 o más restara 1 Punto (En caso de que pierda el partido)
    ...
    # -- ACTIVIDAD 1 --
    #Funcion que sumara dos goles a todos los equipos de casa
    def action_SumDosGolesCasa(self):
        for recordPartido in self.env['liga.partido'].search([]):
            recordPartido.goles_casa = recordPartido.goles_casa + 2
        self.actualizar()

    #Funcion que sumara dos goles a todos los equipos visitantes
    def action_SumDosGolesFuera(self):
        for recordPartido in self.env['liga.partido'].search([]):
            recordPartido.goles_fuera = recordPartido.goles_fuera + 2
        self.actualizar()
